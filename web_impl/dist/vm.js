...
function updateVMButtons() {
    ...
    if (stepBtn)
        stepBtn.disabled = vmRunning;
}
// TODO: Move to isa.ts?
function dissasembleInstruction(inst) {
    ...
    const instruction = Object.values(InstructionInfo).find((info) => info.encoding === opcode && !info.is_directive);
    if (!instruction)
        return "?";
...
function executeNextInstruction(lynxMachine) {
